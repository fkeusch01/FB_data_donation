...
# Date: August 5, 2023
# 
# Data available at https://doi.org/10.7802/2585 
# -----------------------------------------------------------------
...
# Load data
full_data <- read.csv("PINCET_v2-0-0.csv")
...
# RQ1. RQ1.	How willing are Facebook users to donate their data via a designated
# data donation platform for research in a web survey??
...
# RQ1a. What effect does the framing of the data donation request have on willingness
# to donate? 
# Willingness
...
# Successful donation
gl_donation <- table(donation$w4_fb_data_donated, donation$w4_data_don_quest_type)
prop.table(gl_donation, 2)
chisq.test(gl_donation)

# RQ3.	What bias does arise from selective willingness to donate and successful
# donation of Facebook data?
# Willingness
...
# Successful donation
lr_donation <- glm(as.factor(w4_fb_data_donated) ~
                    w4_gender_na +
                    w4_age_cat +
                    w4_edu_cat +
                    w4_fb_use +
...
summary(lr_donation)
PseudoR2(lr_donation, which = NULL)
AME_donation <- margins_summary(lr_donation)
AME_donation
...
AME_donation_fig <- data.frame(AME_donation) %>% 
  mutate(type = factor("Successful donation"),
         variable = factor) %>% 
  select("variable", "AME", "lower", "upper", "type")

AME_empty_don <- data.frame(variable = c("Gender (Ref: Male)",
                                        "Age (Ref: 18-30 years)",
                                        "Educational attainment (Ref: Low)",
                                        "Freq. Facebook use \n(Ref: Several times a month)",
                                        "Framing (Ref: Gain)"),
