...
# Load data
#full_data <- read.csv("PINCET_v1-0-0.csv")
full_data <- `PINCET_v1-0-0`
alt_data <- read.csv("survey_w4.csv")
alt_data <- alt_data %>%
  select(new_id, w4_data_don_quest_type)

open1 <- read.csv("w4_fb_data_don_reason_230601.csv", sep=";")
open2 <- read.csv("w4_fb_data_don_not_success_230601.csv", sep=";")

open1 <- open1 %>% 
  mutate(w4_fb_data_don_reason1 = as.factor(w4_fb_data_don_reason1),
         w4_fb_data_don_reason2 = as.factor(w4_fb_data_don_reason2),
         w4_fb_data_don_reason3 = as.factor(w4_fb_data_don_reason3))

open2 <- open2 %>% 
  mutate(w4_fb_data_don_not_success = as.factor(w4_fb_data_don_success_reason1))
...
donation_task <- merge(donation_task, alt_data, by = "new_id")
donation_task <- merge(donation_task, open1, by = "new_id", all.x = TRUE)
donation_task <- merge(donation_task, open2, by = "new_id", all.x = TRUE)
...
#                                       w4_trust_dt_fb == "0 überhaupt nich" |
#                                       w4_trust_dt_fb == "0 überhaupthaupt " |
#                                       w4_trust_dt_fb == "0 überhpt nicht" |
#                                       w4_trust_dt_fb == "0" |
#                                       w4_trust_dt_fb == "Stimme überhaupt " |
#                                       w4_trust_dt_fb == "0 überhauptn Ger\xc3",
#                                     "0",
...
#                                             w4_trust_dt_research == "Vertraue überhaup",
#                                           "0",
...
OR_m1 <- exp(coef(lr_willing))
se.coef.m1 <- function(m1){sqrt(diag(vcov(lr_willing)))}
Std_Error_OR_m1 <- OR_m1 * se.coef.m1(lr_willing)
...
OR_m2 <- exp(coef(lr_linkage))
se.coef.m2 <- function(m2){sqrt(diag(vcov(lr_linkage)))}
Std_Error_OR_m2 <- OR_m2 * se.coef.m1(lr_linkage)
